...
co_client = cohere.Client("YJKn5iKGlhvfnAFQhXWNgGl7SmAMxHoKa7zCCjWn") #WwhvgMOBIsETi2YGti6DjRLwuZ1OfPJT0fpBoeYf
CHUNK_SIZE = 1024
OUTPUT_BASE_DIR = "./"
...
def process_pdf_file(st_file_objects: Any, run_id: str = None):
    df=''
    for st_file_object in st_file_objects:
        reader = PdfReader(st_file_object)
        for i in range(1,len(reader.pages)):
            df+=reader.pages[i].extract_text()
    
    run_id = get_random_string() if run_id is None else run_id

    output_path = os.path.join(OUTPUT_BASE_DIR, f"{run_id}.pdf")
